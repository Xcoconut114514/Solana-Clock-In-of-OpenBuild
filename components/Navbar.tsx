    ...
    <>
      <nav className="w-full h-16 border-b border-gray-800 bg-black/80 backdrop-blur-md sticky top-0 z-50 flex items-center justify-between px-4 md:px-8">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-400">
          <a href="/" className={location.pathname === '/' ? 'text-white hover:text-green-400 transition-colors' : 'hover:text-white transition-colors'}>Learn</a>
          <a href="#" className="hover:text-white transition-colors">Challenges</a>
          <button
            onClick={() => navigate('/nft-checker')}
            className={`hover:text-white transition-colors flex items-center gap-1.5 ${
              location.pathname === '/nft-checker' ? 'text-white' : ''
            }`}
          >
            <Trophy size={14} />
            NFT徽章
