 ...
 * ---------------------
 * Main card for staking and daily check-in functionality.
 * Integrates with real wallet and stake pool context.
 */
...
import { 
  Lock, ShieldCheck, CalendarCheck, Coins, CheckCircle2, 
  Circle, Loader2, PlayCircle, Trophy, Code2, ExternalLink, 
  Github, BookOpen, AlertCircle, Wallet
} from 'lucide-react';
import { WalletConnectModal } from './WalletConnectModal';

interface CheckInCardProps {
  // Legacy props - now handled via context
  userState?: any;
  onStake?: () => void;
  onConnect?: () => void;
}
...
export const CheckInCard: React.FC<CheckInCardProps> = () => {
  ...
  const { stats, isLoading, error, stake, checkIn } = useStakePool();
  ...
  const renderHeader = () => {
    // User has staked - show check-in interface
    if (stats.userStaked) {
      return (
        <div className="space-y-8 mb-10">
          ...
          <div className="flex items-center justify-between border-b border-gray-800 pb-6">
            ...
            <div className="flex items-center gap-2 bg-green-900/20 border border-green-900/50 px-3 py-1.5 rounded-full">
              ...
              <span className="text-green-400 text-sm font-semibold">已参营 (Joined)</span>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-gray-900 to-black border border-gray-800 rounded-xl p-8 md:p-10 relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="space-y-2 text-center md:text-left">
                ...
                {stats.todayCheckedIn ? (
                  <div className="text-3xl font-bold text-green-400 flex items-center justify-center md:justify-start gap-2">
              ...
              {!stats.todayCheckedIn ? (
                <button 
                  ...
                  disabled={checkInLoading || isLoading}
                  className="w-full md:w-auto px-10 bg-green-400 hover:bg-green-500 text-black font-bold py-4 rounded-xl transition-all shadow-lg shadow-green-900/20 text-xl flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {checkInLoading ? <Loader2 className="animate-spin" /> : <CalendarCheck size={24} />}
                  {checkInLoading ? 'Verifying...' : '立即打卡'}
                </button>
              ) : (
            ...
            {/* Error Display */}
            {error && (
              <div className="mt-4 flex items-center gap-2 text-red-400 text-sm bg-red-900/20 border border-red-900/50 rounded-lg px-4 py-2">
                <AlertCircle size={16} />
                {error}
              </div>
            )}
              ...
              {MOCK_DAYS.map((d) => (
                <div key={d.day} className="flex flex-col items-center gap-2 min-w-[50px]">
                  <div className={`
                    ...
                    ${d.status === 'completed' ? 'bg-green-400/20 border-green-400 text-green-400' : ''}
                    ${d.status === 'missed' ? 'bg-red-900/20 border-red-900 text-red-500' : ''}
                    ${d.status === 'current' ? 'bg-white text-black border-white ring-2 ring-green-400 ring-offset-2 ring-offset-black' : ''}
    ...
    // Default: Not Staked / Disconnected - show join prompt
    return (
          ...
          <div className="flex flex-col gap-4 w-full md:w-auto">
            {!connected ? (
              <button 
                ...
                className="w-full md:w-72 bg-green-400 hover:bg-green-500 text-black font-bold py-4 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 text-lg"
              >
                <Wallet size={20} />
                连接钱包 (Connect)
              </button>
              ...
              <button 
                onClick={handleStake}
                disabled={stakeLoading || isLoading}
                className="w-full md:w-72 bg-green-400 hover:bg-green-500 text-black font-bold py-4 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 text-lg disabled:opacity-50"
              >
                {stakeLoading ? (
                  <>
            ...
            {connected && publicKey && (
              <p className="text-center text-gray-500 text-sm">
                钱包: {publicKey.toBase58().slice(0, 8)}...
              </p>
            )}
  ...
  return (
    <>
      <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
        {/* Top Decor Line */}
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-purple-500 via-green-400 to-green-500"></div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3">
            {COURSE_CONTENT.map((item, index) => (
              <div 
                key={index} 
                className="group bg-black/40 border border-gray-800 hover:border-green-400/50 rounded-xl p-4 transition-all duration-300 hover:bg-white/5 relative overflow-hidden"
              >
                <div className="flex items-start gap-4 relative z-10">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <h4 className="text-white font-medium truncate pr-2 group-hover:text-green-400 transition-colors">
                        {item.title}
                      </h4>
